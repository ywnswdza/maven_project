  ...
  <script type="text/javascript">
  		...
  		window.onload = function(){
				...
				data: {
					...
					onClick : function(event, treeId, treeNode, clickFlag) {
						if(treeNode.isParent) {
							...
							parent.selectRowByTreeClick({nodeText : treeNode.nodeText});
						}
  			...
  			<c:if test="${empty param.kindId}">
  				{nodeText:"所有类别",id : 0,isParent:true,drop : false}//zTree.expandAll(true);
  			</c:if>
  			]);
  		};
		...
		function addTreeNode() {
			...
			if(selectNode) {
				...
				addAndMoidfy = "add";
				parent.f_openWin("添加节点",url,$(window).height() * 0.8 ,800);
			}
		}
		...
		function modifyTreeNode(){
			...
			if(selectNode) {
				...
				addAndMoidfy = "modify";
				parent.f_openWin("修改节点",url,$(window).height() * 0.8 ,800);
			}
		}
