...
//			$(this).each(function(){
				...
				if($ele.attr("id")=='') $ele.attr("pagebarI_",page_obj.indexs);
				var pageP = new page_obj();
				pageP.init($ele,opts);
				return pageP;
//			});
		}
		...
		function page_obj(){
			return {
				indexs : ++page_obj.indexs,
				init : function(element,opts){
					var $this = $(element);
					this.container = $this;
					if(!page_obj.inst) page_obj.inst = {};
					page_obj.inst[$this.attr("id")] = $this;
					this.container._opts = opts;
					methods.toHtml($this);
					methods._bindEvent($this.attr("id"));
					return $this;
				},
				setOptions : function(options){
					this.container._opts = $.extend({},this.container._opts,options || {});
				},
				loadData : function(){
					//重新加载数据
//					methods._generateNav.apply(_pageNav[0],[this._opts]);
					methods.toHtml(this.container);
				}
				,getVersion : function() {
					return _version;
				}
...
//					var $ele = $("#" +id);
					$ele.bind("mousedown",function(e){
						...
						var _opts = $ele._opts; //从上面保存的地方取得参数 
						var $pageBtn = $(srcElem);
						//上一页，事件	
						...
						if(_html == _opts.pervPage_text) {
							...
							_opts.pageNo = parseInt(_currentPage) + 1;
						} else {
							if($pageBtn.hasClass("pageBtnNum") && !$pageBtn.hasClass("moreBtnNum")) {
								if(_opts.pageNo == Number(_html)) return false;
...
//							$this.init.apply($this,[_opts]);
			...
			toHtml : function($ele){
				...
				$ele.addClass("paging");
				$ele.html("");
				$ele.append(htmlArray.join(""));
			},
			_requestData : function($ele){
				var opts = $ele._opts;
				if(opts.async == true) {
					...
					$.ajax({
						...
						success : function(data){
							//todo......................
							$ele._opts.totalPage = data.totalPages < 1 ? 1 : data.totalPages;
