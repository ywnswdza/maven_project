...
(function ($)
{
    ...
    $.ligerDefaults.Tree = {
        ...
        data: null,
        checkbox: true,
        autoCheckboxEven: true, //自动关联选择
        autoParentCheckboxEven : true,  //自动关联选择父节点
        autoChildCheckboxEven : true,  //自动关联选择子节点
        checkIncomplete : false,  //自动关联选择子节点
        parentIcon: 'folder',
        childIcon: 'leaf',
        textFieldName: 'text',
        attribute: ['id', 'url'],
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
                ...
                if ($(obj).hasClass("l-checkbox"))
                {
                    if (p.autoCheckboxEven)
                    {
                        ...
                        if ($(obj).hasClass("l-checkbox-unchecked"))
                        {
                            ...
                            $(".l-children .l-checkbox", treeitem) .removeClass("l-checkbox-incomplete l-checkbox-unchecked")
                        ...
                        if(p.autoChildCheckboxEven == false) g._setChlidCheckboxStatus(treenodedata);
                        if(p.autoParentCheckboxEven == true)  g._setParentCheckboxStatus(treeitem);
                ...
                g.tree.ligerDrag({
                    ...
                    onStopDrag: function (current, e)
                    {
                        ...
                        if (g.nodeDropIn != -1)
                        {
                            ...
                            //TODO drop end
                            if(p.onDragAfter) p.onDragAfter(nodes,g.nodeDropIn);
                            g.nodeDropIn = -1;
                        }
                        g.nodeDroptip.hide();
                        this.set('cursor', 'default');
        ...
        _setChlidCheckboxStatus : function(node){
        	 var treemgr = this;
        	 if(node.children && node.children.length > 0) {
        		 for(var i in node.children) {
        			 treemgr.cancelSelect(node.children[i]);
        			 treemgr._setChlidCheckboxStatus(node.children[i]);
        		 }
        	 }
        }
    });
