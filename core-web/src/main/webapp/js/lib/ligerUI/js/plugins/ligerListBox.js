...
(function ($)
{
    ...
    $.ligerui.controls.ListBox.ligerExtend($.ligerui.controls.Input, {
        ...
        searchByText : function(text,property){
        	var g = this, p = this.options;
        	var searchData = [];
        	var noSearchData = [];
        	for (var i = 0, l = g.data.length; i < l; i++)
            {
                if (g.data[i][p.textField].indexOf(text) != -1) searchData.push(g.data[i]);
                else noSearchData.push(g.data[i]);
            }
        	for(var i = 0;i < searchData.length ; i++) {
        		var _v = searchData[i][p.valueField];
        		$("tr[value='"+ _v +"']",g.selectBox).show();
        	}
        	for(var i = 0;i < noSearchData.length ; i++) {
        		var _v = noSearchData[i][p.valueField];
        		$("tr[value='"+ _v +"']",g.selectBox).hide();
        	}
        ...
        removeItemByValue : function(value){
        	...
        	for(var i = 0 ;i < g.data.length;i++ ) {
        		if(value == g.data[i][p.valueField]){
        			delete g.data.splice(i,1);
        			break;
        		}
        	}
        	g.refresh();
        },
        unSelectByValue : function(value){
        	var g = this, p = this.options; 
        	var  _value = [];
        	if(typeof value === "string") _value = value.split(p.split);
        	else _value = value;
        	$.each(_value,function(index,row){
        		var _v = row;
        		if(typeof row === "object") _v = row[p.valueField];
        		//console.info(_v);
        		$("tr[value='"+ _v +"']",g.selectBox).removeClass("l-selected").find(":checkbox").each(function () { this.checked = false; });
        	});
        },
        ...
        selectItems : function(value){
        	...
        	$.each(_value,function(index,row){
        		...
        		if(typeof row === "object") _v = row[p.valueField];
        		//console.info(_v);
        		$("tr[value='"+ _v +"']",g.selectBox).addClass("l-selected").find(":checkbox").each(function () { this.checked = true; });
        	});
        },
        _dataInit: function ()
        {
        ...
        _addClickEven: function ()
        {
            ...
            {  
                ...
                g._updateValue();
                g.trigger('selected', [value, text, data,jrow.hasClass("l-selected")]);
            });
        } 
    });
